...
function createDownloadLink(blob) {
    ...
    // Upload file to server with POST method
    ...
    xhr.onreadystatechange=function(){
        if (xhr.readyState==4 && xhr.status==200){
            ...
            path_voice_recording = JSON.parse(xhr.responseText);
            ...
            $('#verify').on('click', function() {
                ...
                $.ajax({
                	...
                	data: {
                	    "image": image,
                        "voice_path": path_voice_recording
                    },
                    dataType: 'json',
                	success: function (response) {
                	    console.log(' Success lets call the classifier');
                	    console.log(response);
                        // TODO if not, log the probabilities into the console.
                        // TODO call flask with ajax
                        // TODO redirects to adminpanel
                        $.ajax({
                        	method: "GET",
                        	url: "http://localhost:5000/predict",
                            contentType: "application/json",
                        	success: function (response) {
                                console.log("Fusion was done with following results:");
                                $('#overlay').fadeOut();
                                $('#loader').fadeOut();
                                $('#verify').fadeOut();
                                window.location.replace(baseUrl + 'results.php');
                        	}
                        });
